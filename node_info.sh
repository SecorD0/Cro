#!/bin/bash
# Config
daemon="`which chain-maind`"
token_name="tcro"
node_dir="$HOME/.chain-maind/"
# Default variables
language="EN"
raw_output="false"
# Options
...
while test $# -gt 0; do
	...
	-h|--help)
		...
		echo
		echo -e "${C_LGn}Functionality${RES}: the script shows information about a Cro node"
		echo
		...
		echo -e "https://github.com/SecorD0/Cro/blob/main/node_info.sh - script URL"
...
# Send Pull request with new texts to add a language - https://github.com/SecorD0/Cro/blob/main/node_info.sh
#elif [ "$language" = ".." ]; then
else
...
if [ -n "$cro_wallet_name" ] && [ ! -n "$cro_wallet_address" ]; then
	printf_n "$t_ewa"
	cro_wallet_address=`$daemon keys show "$cro_wallet_name" -a`
	if [ -n "$cro_wallet_address" ]; then
		. <(wget -qO- https://raw.githubusercontent.com/SecorD0/utils/main/miscellaneous/insert_variable.sh) -n cro_wallet_address -v "$cro_wallet_address"
	else
		printf_n "$t_ewa_err"
	fi
fi
...
delegated=`bc -l <<< "$(jq -r ".tokens" <<< $node_info)/100000000"`
...
"$voting_power"
	...
	if [ "$catching_up" = "true" ]; then
		current_block=`wget -qO- "https://testnet-croeseid-4.crypto.org:26657/abci_info" | jq -r ".result.response.last_block_height"`
	...
	if [ -n "$cro_wallet_address" ]; then
		printf_n "$t_wa" "$cro_wallet_address"
		balance=`bc -l <<< "$($daemon query bank balances "$cro_wallet_address" -o json --node "$node_tcp" | jq -r ".balances[0].amount")/100000000"`
